...
"""
Program for finding shortest path given n goals on the Europe map from Ticket To Ride.
...
def visualizePath(G, path):
    ...
    for i in range(len(path)-1):
        G.add_edge(path[i], path[i+1], color='r') # add color with color='r'
    ...
    for i in range(len(path)-1):
        G.add_edge(path[i], path[i+1], color='g')
    ...
    for node1, edges in graph.items():
        for node2, weight in edges.items():
            if not G.has_edge(node1, node2) or not G.has_edge(node2, node1): # skip duplicates
                G.add_edge(node1, node2, weight=weight, color='g')
